...
import type {
  QueryProcessingInput,
  QueryProcessingResult,
  UserQueryApiResponse,
} from './queryResultUtils';
import { buildQueryResult } from './queryResultUtils';
...
export interface PersistQueryResultArgs {
  ...
  allPriorResults: QueryProcessingResult[];
}
...
export async function persistOneQueryResult(
  ...
  updatedAllResults: QueryProcessingResult[];
}> {
  const queryResult = buildQueryResult(args);
  const updatedAllResults: QueryProcessingResult[] = [...args.allPriorResults, queryResult];
