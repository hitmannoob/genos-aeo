...
describe('OpenRouter provider routing', () => {
  it('lets Perplexity Sonar use native search without a server-tool payload', async () => {
  ...
  it('still sends the web-search server tool to Google', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(
      JSON.stringify({
...
describe('Google grounding citation domains', () => {
  ...
  it('uses the Google-provided source label as the domain while retaining the redirect URL', () => {
    ...
    expect(transformed.aiOverviewReferences).toEqual([
      expect.objectContaining({ url: redirectUrl, domain: 'nykaa.com' }),
    ]);
  });

  it('repairs previously stored redirect citations from their source label', () => {
  ...
  it('does not pretend an article title is a source domain', () => {
